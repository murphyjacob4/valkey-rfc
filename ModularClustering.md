...
- Module Heartbeats: Modules send heartbeats to the Core periodically to inform
  that the cluster is in "OK" state. The Core does not understand or care what
  is happening, it just listens and reacts.
...
typedef struct ClusterOps {
   ...
   /* Defines a single update to metadata, whether within the global Directory
    * or within the per-shard Log. */
   ...
   /* Update one or many pieces of global metadata atomically.
    *
    * Global metadata is located in the "global Directory". The Directory does
    * not guarantee durability (i.e. eventual gossip, CRDT-ish semantics).
    *
    * Upon success or failure, call the provided Callback function. Core will
    * handle it as needed. */
   int (*UpdateGlobalDirectory)(ValkeyModuleCtx *ctx, size_t num_ops,
                                ValkeyModuleClusterMetadataOperation []ops,
                                UpdateMetadataCallback callback);

   /* Update one or many pieces of shard-level metadata atomically.
    *
    * Shard-level metadata is located in the "shard Log". The Log guarantees
    * durability as long as the metadata operation is successful (quorum
    * semantics).
    *
    * Upon success or failure, call the provided Callback function. Core will
    * handle it as needed. */
...
1. **Global Directory** - cluster-level metadata that provides no strong
   consistency or durability (similar to Cluster v1 gossip).
2. **Shard Log** - shard-level metadata that provides strong consistency and
   durability (like Cluster v1 elections, but with stronger guarantees).

The Directory and Log are two completely separate namespaces. Modules are free
to implement these how they see fit, and could even use the same underlying
storage for both. But the important parts are:

- Modules do not need to implement global consensus.
- Modules can optimize the Global Directory to prevent needless bottlenecking.
...
| Shard Primaryship (routing) | GLOBAL DIRECTORY | Directory has one owner per shard at a time and will eventually converge to one view cluster-wide.                                                           |
...
| Slot Routing Table          | GLOBAL DIRECTORY | Directory has one owner per slot at a time, and will eventually converge to one view cluster-wide.                                                           |
...
| Node Port/IP                | GLOBAL DIRECTORY | No consensus needed, but must be broadcast globally. Okay if it takes some time to propagate.                                                                |
| Replication Offset          | GLOBAL DIRECTORY | Node-level metadata that shard members need to know about for proper failover, but it doesn't need consensus and is okay if it is lost on restart/failover.  |
...
```
/* Triggered when new metadata is detected in the global directory. */
void VM_ClusterProviderNotifyGlobalDirectoryUpdate(ValkeyModuleCtx *ctx,
                                                  size_t num_ops,
...
#### 3.3.3 Global Directory Conflict Resolution

Conflicts may arise when two nodes make conflicting global Directory metadata
changes (e.g. two shards try to claim unowned slot 1000).
...
`VM_ClusterProviderNotifyGlobalDirectoryUpdate` with the winning value.
...
The module must propagate all Shard Log data to all shard members eventaully,
and all Global Directory to all cluster members eventually.

#### 3.3.5 Local Restart Persistence

Upon local restart, the Module should re-bootstrap into the cluster during
module load time. Before the module can claim the cluster is in an "OK" state,
it needs to sync up to the latest Shard Log. If the re-bootstrapping node is to
resume primaryship, the synced view of the Shard Log should guarantee with
confidence that primaryship has not been transferred elsewhere. This would
commonly be implemented via lease, but the details are opaque to the Core.

What is persisted to disk to ensure the Module re-bootstrap back into the
cluster is opaque to the Core and owned by the Module.
...
| Global Directory Writes      | All writes go to a single global Raft log                                        | All writes apply to the local view of global Directory with incremented logical version                           | All writes go to the external store                         |
| Global Directory Conflicts   | N/A                                                                              | Conflicts deterministically handled when two nodes gossip a key with different values on the same logical version | N/A                                                         |
| Global Directory Propagation | All nodes are members of the global Raft group, trigger callbacks when committed | Gossip spreads the current view, merged with each node's local view                                               | Nodes watch and/or poll keys in external store              |
...
Slot assignment is broken into two metadata pieces: one in the Global Directory
and one in the Shard Log:

- The Global Directory metadata we will refer to as the "Slot Routing Table" and
  is gossipped throughout the cluster for the purposes of routing.
...
1. Shard A and Shard B both `UpdateGlobalDirectory` claiming "Slot 1000" in the
   Slot Routing Table.
...
4. **Resolution:**
   ...
   - The Loser (Shard B): Sees the conflict and knows it lost. It must:
     - Fire
       `VM_ClusterProviderNotifyGlobalDirectoryUpdate(["slot1000:owner", "shard_A", <revision>])`
       so the Core updates its routing table.
     - Upon notification that the global routing was reverted, the Core needs to
       do corrective actions to remove the slot assignment from the shard
       (remove Shard Slot Ownership) and delete the dirty keys.
...
```mermaid
sequenceDiagram
    participant Global as Global Directory
    ...
    par Concurrent Writes
        ShardA->>Global: Publish: Slot 1000 -> Shard A
        ShardA->>ShardA: Ownership: ACTIVE
        and
        ShardB->>Global: Publish: Slot 1000 -> Shard B
        ShardB->>ShardB: Ownership: ACTIVE
    end

    Note over ShardA, ShardB: Propagation & Detection

    Global-->>ShardB: Receive Update (Slot 1000 -> Shard A)
    ...
    ShardB->>ShardB: Update Global View -> Shard A
    ShardB->>ShardB: Remove Ownership
    Note right of ShardB: Shard B stops serving Slot 1000

    Global-->>ShardA: Receive Update (Slot 1000 -> Shard B)
    ...
    ShardA->>ShardA: Maintain Global View -> Shard A
...
sequenceDiagram
    ...
    Note over Target, Global: Initial State: Slot 1000 owned by Shard A
...
sequenceDiagram
    ...
    Note over Global, ShardB: Initial State: Slot 1000 owned by Shard A
    Global->>Global: Slot 1000 -> Shard A
    ...
    Note over Global: Step 3: Switch Routing
    ShardB->>Global: Update Slot 1000 -> Shard B
    Global-->>ShardA: Propagate Update (Slot 1000 -> Shard B)

    Note over ShardA: Step 4: Unfence & Redirect
    ShardA->>ShardA: See Global Update -> Remove Ownership
...
sequenceDiagram
    ...
    Note over Global, ShardB: Initial State: Migration Started
    Global->>Global: Slot 1000 -> Shard A
    ShardB->>ShardB: Slot 1000 Ownership: TAKING_OVER:ShardA
    ...
    ShardA->>Global: Slot 1000 -> Shard B
    ...
    ShardB->>Global: Syncs Global Table (Sees Slot 1000 -> Shard B)
...
sequenceDiagram
    ...
    par Race to Claim
        ReplicaA->>Metadata Store: Update Primary="ReplicaA"<br/>(Rev: 10)
        ReplicaB->>Metadata Store: Update Primary="ReplicaB"<br/>(Rev: 10)
    end

    Note over Metadata Store: ReplicaA wins CAS
    Metadata Store-->>ReplicaA: Success (New Rev: 11)
    Metadata Store-->>ReplicaB: Fail (Stale Revision)

    Note over ReplicaA, ReplicaB: Propagation
    Metadata Store->>ReplicaA: Notify(Primary="ReplicaA")
    Metadata Store->>ReplicaB: Notify(Primary="ReplicaA")

    ReplicaA->>ReplicaA: Promote to Primary
    ReplicaB->>ReplicaB: Follow ReplicaA

    Note over Primary: Back Online
    Primary->>Metadata Store: Sync with Quorum
    Metadata Store->>Primary: Notify(Primary="ReplicaA")
...
sequenceDiagram
    ...
    OldPrimary->>Metadata Store: Update Primary="Node A:FENCED"
    ...
    NewPrimary->>Metadata Store: Update Primary="Node B"

    Metadata Store->>OldPrimary: Notify(Primary="Node B")
    OldPrimary->>OldPrimary: Demote to Replica, Redirect Fenced Clients

    Metadata Store->>NewPrimary: Notify(Primary="Node B")
...
UpdateShardLog(...,
               ["primary", "node_A", <revision>]
               ...);
```

**Why this is safe**: Unlike Slot Migration (which spans two consensus groups),
Failover occurs within a single `SHARD` consensus scope. If Node A attempts to
...
sequenceDiagram
    ...
    Replica->>Metadata Store: Update Primary="Node A:FENCED"
    Metadata Store->>Primary: Notify(Primary="Node A:FENCED")
    ...
    Primary->>Metadata Store: Update Primary="Node A" (Revert)

    alt Success (Rollback)
        Metadata Store-->>Primary: Success
        ...
        Replica->>Metadata Store: Update Primary="Node B"
        Metadata Store-->>Replica: Fail (Revision Mismatch)
    end
```
